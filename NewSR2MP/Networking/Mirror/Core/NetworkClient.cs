    ...
    public static partial class NetworkClient
    {
        ...
        internal static void OnTransportData(ArraySegment<byte> dataRaw, int channelId)
        {
            if (connection != null)
            {
                       ...
                       unbatcher.GetNextMessage(out ArraySegment<byte> message, out double remoteTimestamp))
                {
                    ...
                    {
                        if (false /*please remove if found*/) SRMP.Log($"Message b{b}");
                    }
            ...
            where M : struct, NetworkMessage
        {
            using (NetworkWriterPooled writer = NetworkWriterPool.Get())
            {
                ...
                if (false /*please remove if found*/) SRMP.Log(NetworkMessageId<M>.Id.ToString());
                ...
                if (false /*please remove if found*/) SRMP.Log("Written! Sending.");
                
                (Transport.active as EpicTransport.EosTransport).ClientSend(writer.ToArraySegment(), channelId);
