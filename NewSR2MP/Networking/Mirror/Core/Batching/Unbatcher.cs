...
namespace Mirror
{
    public class Unbatcher
    {
        ...
        public bool GetNextMessage(out ArraySegment<byte> message, out double remoteTimeStamp)
        {
            ...
            message = default;
            remoteTimeStamp = 0;
            if (false /*please remove if found*/) SRMP.Log($"batch count: {batches.Count}");
            if (false /*please remove if found*/) SRMP.Log($"reader cap: {reader.Capacity}");
            ...
            {
                // retire the batch
                if (false /*please remove if found*/) SRMP.Log("retire batch");
                ...
                if (false /*please remove if found*/) SRMP.Log("next batch");
            ...
            if (false /*please remove if found*/) SRMP.Log($"buffer size: {reader.buffer.Count}");
            if (false /*please remove if found*/) SRMP.Log($"buffer pos : {reader.Position}");
            ...
            if (false /*please remove if found*/) SRMP.Log($"set remote time stamp");

            if (false /*please remove if found*/) SRMP.Log($"reader remaining: {reader.Remaining}");
            ...
            try {
                ...
                if (false /*please remove if found*/) SRMP.Log($"used {bytesUsed} byte(s) in decompression.");
                reader.Position = (8 + 2);
            ...
            if (false /*please remove if found*/) SRMP.Log("finish batch");
