...
namespace Mirror
{
    ...
    public class NetworkReader
    {
        ...
        // some functions can allocate a new byte[], Il2CppSystem.Collections.Generic.List<T>, Texture, etc.
            ...
            where T : unmanaged
        {
            ...
            Position += size;
            if (false /*please remove if found*/) SRMP.Log($"new Byte at pos {Position}: {buffer.Array[Position]}; Stack Trace: {StackTraceUtility.ExtractStackTrace()}");
            return value;
        }
        ...
        public ArraySegment<byte> ReadBytesSegment(int count)
        {
            ...
            if (Remaining < count)
            {
                if (false /*please remove if found*/) SRMP.Log($"ReadBytesSegment can't read {count} bytes because it would read past the end of the stream. {ToString()}");
            }
