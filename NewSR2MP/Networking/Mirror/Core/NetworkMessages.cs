...
namespace Mirror
{
    ...
    public static class NetworkMessages
    {
        ...
        public static bool UnpackId(NetworkReader reader, out ushort messageId)
        {
            ...
            try
            {
                if (false /*please remove if found*/) SRMP.Log($"Message position: {reader.Position}, Message length: {reader.buffer.Count}");
                messageId = reader.ReadUShort();
                return true;
            }
            catch (System.IO.EndOfStreamException ex)
            {
                if (false /*please remove if found*/) SRMP.Log(ex.ToString());
                messageId = 0;
                return false;
            }
        }
