...
namespace EpicTransport {   
    public abstract class Common {
        ...
        protected void WaitForClose(ProductUserId clientUserID, SocketId socketId) => MelonCoroutines.Start(DelayedClose(clientUserID, socketId));
        ...
        public void ReceiveData() {
            try
            {
                ...
                while (transport.enabled && Receive(out ProductUserId clientUserID, out socketId,
                           out byte[] internalMessage, (byte)internal_ch))
                {
                ...
                {
                    while (transport.enabled && Receive(out ProductUserId clientUserID, out socketId,
                               out byte[] receiveBuffer, (byte)chNum))
                    {
                ...
                foreach (KeyValuePair<PacketKey, List<List<Packet>>> keyValuePair in incomingPackets)
                {
                    for (int packetList = 0; packetList < keyValuePair.Value.Count; packetList++)
                    {
                        ...
                        for (int packet = 0; packet < keyValuePair.Value[packetList].Count; packet++)
                        {
                            ...
                            if (tempPacket.fragment != packet || (packet == keyValuePair.Value[packetList].Count - 1 &&
                                                                  tempPacket.moreFragments))
                            {
                                packetReady = false;
                            }
                            else
                        ...
                        if (packetReady)
                        {
                            ...
                            for (int packet = 0; packet < keyValuePair.Value[packetList].Count; packet++)
                            {
                                Array.Copy(keyValuePair.Value[packetList][packet].data, 0, data, dataIndex,
                                    keyValuePair.Value[packetList][packet].data.Length);
